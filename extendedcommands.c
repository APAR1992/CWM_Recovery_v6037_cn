...
void show_install_update_menu()
{
    ...
    if (volume_for_path("/emmc") != NULL) {
        other_sd = "/emmc/";
        install_menu_items[3] = "从内置SD卡选择zip刷机包";
    }
    else if (volume_for_path("/external_sd") != NULL) {
        other_sd = "/external_sd/";
        install_menu_items[3] = "从外置SD卡选择zip刷机包";
    }
...
void show_choose_zip_menu(const char *mount_point)
{
    ...
    static const char* headers[] = {  "选择zip刷机包",
                                "",
                                NULL
    };
...
void show_nandroid_restore_menu(const char* path)
{
    ...
    static const char* headers[] = {  "选择要还原的备份",
                                "",
                                NULL
    };
...
void show_nandroid_delete_menu(const char* path)
{
    ...
    static const char* headers[] = {  "选择要删除的备份",
                                "",
                                NULL
    };
...
int control_usb_storage_for_lun(Volume* vol, bool enable) {
    static const char* lun_files[] = {
        ...
        "/sys/devices/platform/mt_usb/gadget/lun%d/file",//for mtk USB-mount
...
void show_mount_usb_storage_menu()
{
    ...
    static const char* headers[] = {  "已进入USB大容量存储",
                                "离开此菜单时",
                                "SD卡会从PC端弹出.",
                                "",
                                NULL
    ...
    for (;;)
    {
        ...
        if (!is_data_media()) {
          ...
          options[mountable_volumes + formatable_volumes + 1] = NULL;
            ...
            options[mountable_volumes + formatable_volumes + 1] = "挂载USB存储";
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static const char* advancedheaders[] = {  "选择一个备份来还原",
                                "",
                                "先选择一个备份",
                                "接下来的菜单中",
                                "会显示更多选项",
...
void process_volumes() {
    ...
    ui_set_show_text(1);
    ui_print("文件格式需要转换为Ext4格式.\n");
    ui_print("即将开始备份和还原.\n");
    ui_print("如果发生错误，你的备份将被\n");
    ui_print("命名为 %s. 尝试恢复 \n", backup_name);
    ui_print("以防错误.\n");
...
int verify_root_and_recovery() {
    ...
    if (0 != lstat("/system/etc/.installed_su_daemon", &st)) {
        ...
        if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
            if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("系统将会在重启时恢复原厂Recovery.禁止恢复?", "是 - 禁止恢复原厂Recovery")) {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("可能会丢失Root权限.确认修复?", "确认 - 修复Root(/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("可能会丢失Root权限.确认修复?", "确认 - 修复Root(/system/xbin/su)")) {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("Root权限丢失. Root设备?", "确认 - Root设备(/system/xbin/su)")) {
