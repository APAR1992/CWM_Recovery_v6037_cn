...
//        return ret;    
    ...
    if (vol == NULL || 0 != stat(vol->device, &s))
    {
        ui_print("未找到sd-ext.跳过备份.\n");
    }
    else
    {
        if (0 != ensure_path_mounted("/sd-ext"))
            ui_print("无法挂载sd-ext. sd-ext备份可能不支持此设备.正在跳过sd-ext备份.\n");
...
//    	return ret;
    ...
    if (restore_wimax && vol != NULL && 0 == stat(vol->device, &s))
    {
        ...
        {
            ui_print("在还原前擦除 WiMAX ...\n");
            if (0 != (ret = format_volume("/wimax")))
                return print_and_error("格式化 wimax 出错!\n");
            ui_print("正在还原 WiMAX ...\n");
            if (0 != (ret = restore_raw_partition(vol->fs_type, vol->device, tmp)))
                return ret;
        }
